...
<!-- ::: {#exr-tactile-birthday-data}
Recall the birthday problem of @exm-birthday.
Let $B$ be the event that at least two people in a group of $n$ share a birthday.


Now assume distribution of birthdays follows this (read in data)
...
### Independent Uniform model {#sec-meeting-sim-independent-uniform}
...
### Independent Normal model {#sec-meeting-sim-independent-normal}
...
<!-- Add plot here that just plots the p-hats by row, withouth the CIs, then say what happens if we add or subtract moe? -->
...
### Normal distributions (briefly) {#sec-normal-briefly}
...
Quantities like expected value, variance, and standard deviation summarize characteristics of the *marginal* distribution of a single random variable.
...
Consider the independent Uniform(0, 60) model of arrival times in the meeting problem from @sec-meeting-sim-independent-uniform.
Let $R$ and $Y$ be the two arrival times, and let $T=\min(R, Y)$ and $W=|R-Y|$ be the first arrival time and waiting time random variables, respectively. 
Our goal is to compute the covariance between $T$ and $W$.

We'll illustrate the calculation using the simulation results from @tbl-meeting-tactile-uniform-sim-table-pair.
The simulated $(T, W)$ pairs are displayed in @tbl-sim-cov-calc.
The mean of the 10 simulated values of $T$ is `{r} round(mean(meeting_tactile_sim[, "T"]), 1)`, and the mean of the 10 simulated values of $W$ is `{r} round(mean(meeting_tactile_sim[, "W"]), 1)`.
We want to measure the degree to which $T$ and $W$ "co-deviate from the their respective means".
For each value of $T$ we find its deviation from the mean, and likewise for $W$; see the "T_deviation" and "W_deviation" columns of @tbl-sim-cov-calc.
Notice that we do care about the sign since we are interested in the *relationship* between $T$ and $W$, which involves questions like: if $T$ is above the mean, does $W$ tend to be above or below the mean?
For each simulated $(T, W)$ pair, we now have a pair of deviations from the respective mean.

We want to measure the degree of co-deviation between $X$ and $Y$; one way to do this is to measure the interaction between the paired deviations by taking their *product*.
See the "product_of_deviations" column of @tbl-sim-cov-calc.
Within each row, the value in the product column is the product of the values in the "T_devation" and "W_deviation" columns.
The *covariance* is the *average of the product of the deviations*: sum the 10 values of the product and divide by 10 to get a covariance of `{r} round(cov(meeting_tactile_sim[, "W"], meeting_tactile_sim[, "T"]) * 9 / 10, 1)`.
$$
...
```{r}
#| label: tbl-sim-cov-calc
#| echo: false
#| tbl-cap: "Calculation of covariance based on 10 simulated values"

cov_calc_df = meeting_tactile_sim |>
  rename(repetition = 1) |>
  mutate(repetition = factor(repetition)) |>
  select(-c(R, Y)) |> 
  mutate(
    T_deviation = T - mean(T),
    W_deviation = W - mean(W),
    product_of_deviations = T_deviation * W_deviation
)
...
Why take the product of the deviations?
...
@fig-cov-calc-rectangles visualizes the covariance calculation.
The ten $(T, W)$ pairs from @tbl-sim-cov-calc are displayed in each plot.
The green "+" represents the two means.
@fig-cov-calc-rectangles-1 highlights the first row of @tbl-sim-cov-calc.
The horizontal orange segment represents the deviation of the $T$ value from the mean of the $T$ values, and the vertical blue segment represents the deviation of the $W$ value from the mean of the $W$ values.
If we think of these deviations as the sides of the shaded rectangle, the area of the rectangle represents the magnitude of the product of the deviations.
For this pair the product would be negative since the value of $T$ is above the mean but the value of $W$ is below the mean.
@fig-cov-calc-rectangles-2 displays ten rectangles, one for each $(T, W)$ pair.
The rectangles are shaded grey if the product of the deviations is negative, and yellow if positive.
The covariance is the average of the ten areas, keeping in mind that the areas can represent either positive (yellow) or negative (gray) values.
There is more gray area than yellow area, so the covariance is negative.


```{r}
#| label: fig-cov-calc-rectangles
#| echo: false
#| fig-cap: "Illustration of covariance calculation for the values in @tbl-sim-cov-calc"
#| fig-subcap: 
#|   - "A single point with its paired deviations and product rectangle"
#|   - "Paired deviations and product rectangles for all points"
#| layout-ncol: 2

cov_calc <- meeting_tactile_sim |> 
  mutate(x = T, y = W) |>
  mutate(x_dev = x - mean(x),
  y_dev = y - mean(y),
  dev_prod = x_dev * y_dev,
  dev_pos = dev_prod > 0,
  xmin = pmin(x, mean(x)),
  ymin = pmin(y, mean(y)),
  xmax = pmax(x, mean(x)),
...
x_1 = meeting_tactile_sim[1, "T"]
y_1 = meeting_tactile_sim[1, "W"]

x_mean = mean(meeting_tactile_sim[, "T"])
y_mean = mean(meeting_tactile_sim[, "W"])
...
aes(x = x, y = y)) +
  geom_point() +
  labs(x = "T", y = "W") + 
  annotate("rect", xmin = x_mean, xmax = x_1, ymin = y_1, ymax = y_mean, alpha = 0.5, color = "#999999") +
  annotate("segment", x = x_1, xend = x_1, y = y_mean, yend = y_1, color = "#56B4E9", linewidth = 1.2) + 
  annotate("segment", x = x_mean, xend = x_1, y = y_1, yend = y_1, color = "#E69F00", linewidth = 1.2) + 
  annotate("point", x = x_mean, y = y_mean, color = "#009E73", shape = 3, size = 3, stroke = 2) + 
  theme_classic() +
  theme(legend.position = "none")
...
ggplot(cov_calc,
aes(x = x, y = y, fill = dev_pos)) +
  geom_point() +
  labs(x = "T", y = "W") + 
  geom_rect(aes(xmin = xmin, ymin = ymin, xmax = xmax, ymax = ymax), alpha = 0.5) +
  scale_fill_manual(values = c(`TRUE` = "#F0E442", `FALSE` = "#999999")) +
  geom_segment(aes(x = x, xend = x, y = ymin, yend = ymax), color = "#56B4E9", linewidth = 1.2) + 
  geom_segment(aes(x = xmin, xend = xmax, y = y, yend = y), color = "#E69F00", linewidth = 1.2) + 
  annotate("point", x = x_mean, y = y_mean, color = "#009E73", shape = 3, size = 3, stroke = 2) + 
  theme_classic() +
  theme(legend.position = "none")
```
...
@tbl-sim-cov-calc and @fig-cov-calc-rectangles motivate the idea and calculation of covariance, but we're really interested in what happens in the long run.
We'll start by simulating many $(T, W)$ pairs.
...
We can do this subtraction in a "vectorized" way: the following code subtracts the first component of `t_and_w.mean()` (the average of the $T$) from the first component of each of the `t_and_w` values (the individual $t$ value), and similarly for the second.
...
This value is the *covariance*, which we could have arrived at more quickly by calling `cov` on the simulated $(T, W)$ pairs.
...
The covariance between $T$ and $W$ is negative because the two variables have an overall negative association.
The largest values of $T$ (near 60) are associated with the smallest values of $W$ (near 0) and vice versa.
Since both people need to arrive in the 0 to 60 time interval, if the first person arrives very late then they won't have to wait very long.
Note that the association isn't perfect; the smallest values of $T$ (near 0) are associated with the whole range of waiting times (from 0 to 60).
But the *overall tendency* is a negative association.


:::: {.callout-note appearance="simple"}
::: {#exm-covariance-arrivals-meeting}
Now consider the arrival times $R$ and $Y$ of the two people in the meeting problem.

1.  Explain in words in full detail how you could simulation to approximate the covariance between $R$ and $Y$.
Assume that $R$ and $Y$ following the independent Uniform(0, 60) model of @sec-meeting-sim-independent-uniform
1.  For each of the following, explain if the covariance between $R$ and $Y$ will be positive, negative, or close to 0.
Then use Symbulate to approximate the covariance.
    -   the independent Uniform(0, 60) model of @sec-meeting-sim-independent-uniform
    -   the independent Normal(30, 10) model of @sec-meeting-sim-independent-normal
    -   the Bivariate Normal model of @sec-language-simulation-computer-meeting-bvn
...
:::: {.callout-tip collapse=true}
::: {#sol-covariance-arrivals-meeting}
1.  Spin the Uniform(0, 60) spinner twice to simulate an $(R, Y)$ pair.
Repeat to simulate many $(R, Y)$ pairs.
Compute the average of the simulated $R$ values and the deviations of the $R$ values from their average; likewise for $Y$.
Now you have a pair of deviations for each repetition; compute the product of the paired deviations and then average the products to approximate the covariance between $R$ and $Y$.
(The only difference for the other models in the next part would be how you simulate and $(R, Y)$ pair; once you have the pairs you would approximate the covariance using the same method.)
1.  We basically assumed the covariance in definining the model for $R$ and $Y$
    -   $R$ and $Y$ are independent, so there will be no overall association and we expect the covariance to be near 0
    -   $R$ and $Y$ are independent, so there will be no overall association and we expect the covariance to be near 0.
    The pattern of variability of $(R, Y)$ would be different than in the previous part but there is still no relationship between $R$ and $Y$.
    -   We expect the covariance to be positive because we assumed that there was a positive association---they tend to arrive around the same time---when definiing the model.
:::
::::
...
The following definiton states our notion of covariance in expected value terms.
...
In this way
$$
\E((X-\E(X))(Y-\E(Y))) = \text{Long Run Average of} ((X - \text{Long Run Average of }X)(Y - \text{Long Run Average of }Y))
$$
where the average is over many $(X, Y)$ *pairs*.


Unfortunately, the actual *value* of covariance is hard to interpret as it depends heavily on the measurement unit scale of the random variables.
In the meeting problem, if $T$ and $W$ are both measured in minutes then the covariance is measured in minutes $\times$ minutes, e.g., -100 $\text{minutes}^2$.
If we converted the $T$ values from minutes to seconds, then the value of covariance would change accordingly and would be measured in minutes $\times$ seconds, e.g., -6000 minutes $\times$ seconds.
In another context, if the covariance between height (m) and weight (kg) is 0.3 $\text{m}\times\text{kg}$, then the covariance between height (cm) and weight (g) is 300,000 $\text{cm}\times\text{g}$.
...
We discussed how variance has many nice theoretical properties, but standard deviation is easier to interpret.
Similarly, covariance has many nice theoretical properties, but *correlation* is easier to interpret. 
We will introduce correlation in the next subsection.

Recall that for discrete random variables we can compute expected values as probability-weighted average values.
We can compute covariances for discrete random variables in a similar way.



:::: {.callout-note appearance="simple"}
::: {#exm-cov-compute-dice}
In the dice rolling problem, let $X$ be the sum and $Y$ be the larger of the two rolls.
The joint distribution of $X$ and $Y$ is represented[^joint-flat] by @tbl-dice-joint-dist-flat.
In @exm-dice-ev-intro we computed $\E(X) = 5$ and $\E(Y) = 3.125$.

[^joint-flat]: We usually represent joint distributions as two-way tables, but for this problem it helps to think of a table with one row for each pair.

1.  Will $\Cov(X, Y)$ be positive, negative, or close to 0?
Explain without computing.
1.  Compute $\Cov(X, Y)$.
1.  Describe in detail how $\Cov(X, Y)$ can be simulated via approximation
1.  Write Symbulate code to approximate $\Cov(X, Y)$ and compare to the values from part 2.
:::
::::

:::: {.callout-tip collapse=true}
::: {#sol-cov-compute-dice}

1.  The covariance will be positive.
The smallest value of $Y$ only occurs with the smallest value of $X$, and when $Y$ is its largest value $X$ is above average.
1.  For the pair (2, 1), which occurs with probability (1/16) the paired deviations are $(2-5, 1 - 3.125)$, the product is $(2-5)(1-3.125)$, and so the contribution to the probability-weighted average is $(2-5)(1-3.125)(1/16) = 0.3984$; see the first row of @tbl-sd-compute-dice.
We repeat for each possible $(X, Y)$ pair; see @tbl-sd-compute-dice for more detail.
The covariance is the sum of the values in the last column, $\Cov(X, Y) = 1.25$.
1.  We have already discussed how to simulate $(X, Y)$ pairs in this content.
To approximate the covariance
    -   Simulate many $(X, Y)$ pairs
    -   Compute the average of the simulated values of $X$ (to approximate $\E(X)$), and likewise for $Y$
    -   From each simulated value of $X$ subtract the average, and likewise for $Y$
    -   Compute the product of the two deviations from the previous part for each simulated pair
    -   Average the products from the previous part to approximate the covariance
1.  See below for the code, both the long way and using the `cov` shortcut.
The simulation-based approximation is close to 1.25.
...
```{r}
#| label: tbl-cov-compute-dice
#| echo: false
#| tbl-cap: "Steps in computing the covariance of $X$ and $Y$ in @exm-cov-compute-dice."
...
knitr::kable(
  data.frame(x, y, x_dev, y_dev, dev_prod, p, p * dev_prod),
  col.names = c("$x$", "$y$", "$x - \\E(X)$", "$y - \\E(Y)$", "$(x - \\E(X))(y - \\E(Y))$", "$\\IP(X=x, Y=y)$", "$(x - \\E(X))(y - \\E(Y))\\IP(X=x,Y=y)$"),
  booktabs = TRUE,
  digits = 4
)
...
x_deviations * y_deviations
```

```{python}
(x_deviations * y_deviations).mean()
```
...
:::: {.callout-note appearance="simple"}
::: {#exm-cov-compute-dice2}
Continuing @exm-cov-compute-dice.

1.  Compute $\E(XY)$.
(We'll see why very soon.)
Is it equal to $\E(X)\E(Y)$?
1.  Describe in detail how $\E(XY)$ can be approximated via simulation.
1.  Compute $\E(XY) - \E(X)\E(Y)$.
What do you notice?
:::
::::

:::: {.callout-tip collapse=true}
::: {#sol-cov-compute-dice2}

1.  If $X$ is 5 and $Y$ is 3, which occurs with probability 2/16, then the product $XY$ is (5)(3) = 15.
To compute $\E(XY)$ as a probability-weighted average value, multiply each possible of $XY$ by its probability and sum.
$$
{\scriptsize
(2)(1)(1/16)+(3)(2)(2/16)+(4)(2)(1/16)+(4)(3)(2/16)+(5)(3)(2/16)+(5)(4)(2/16)+(6)(3)(1/16)+(6)(4)(2/16)+(7)(4)(2/16)+(8)(4)(1/16)
}
$$
$\E(XY) = 16.875$.
Notice this is *not* the same as $\E(X)\E(Y)=(5)(3.125) = 15.625$
1.  We have already discussed how to simulate $(X, Y)$ pairs in this content.
To approximate $\E(XY)$
    -   Simulate many $(X, Y)$ pairs
    -   For each simulated $(X, Y)$ pair compute the product $XY$.
    For example, if $X$ is 5 and $Y$ is 3 then the product $XY$ is (5)(3)=15.
    -   Average the products from the previous part to approximate $\E(XY)$
See the Symbulate code below.
1.  $\E(XY) - \E(X)\E(Y) = 16.875-(5)(3.125) = 1.25$, which is $\Cov(X, Y)$.
...
```{python}
(x_and_y[0] * x_and_y[1]).mean()
```


```{python}
(x_and_y[0] * x_and_y[1]).mean() - x_and_y[0].mean() * x_and_y[1].mean()
```

@exm-cov-compute-dice2 illustrates an alternative formula for computing covariance.
@def-covariance represents the concept of covariance.
However, covariance is usually computed using the following equivalent but slightly simpler formula.

::: {#lem-covariance-shortcut}
$$
...
We will see how to compute covariance for continuous random variables later.
But @def-covariance and @lem-covariance-shortcut apply for both discrete and continuous random variables.
Furthermore, covariance can be approximated via simulation in the same way for discrete or continuous random variables: simulate many pairs of the random variables and compute the average of the products of the paired deviations from the respective means.
...
1.  $W$ and $Z$. Hint: remember that there are two rolls.
...
1.  Positive. 
If $W$ is large (roll many 4s) then $X$ (sum) tends to be large.
1.  Negative. 
If $Z$ is large (roll many 1s) then $X$ (sum) tends to be small.
1.  Zero. 
Basically, the positive association between $X$ and $W$ cancels out with the negative association of $X$ and $Z$.
$V$ is large when there are many 1s or many 4s, or some mixture of 1s and 4s.
So knowing that W is large doesn't really tell you anything about the sum.
1.  Negative.  
There is a fixed number of rolls.
If you roll lots of 4s ($W$ is large) then there must be few rolls of 1s ($Z$ is small).
...
Recall that we can standardize a random variable to put in on a standard measurement scale in which values are measured in terms of standard deviations above or below the mean.
It is useful to standardize random variables when measuring their co-deviation, measuring the deviations in standard deviation terms rather than measurement units.
The correlation between two random variables is the covariance between the corresponding standardized random variables.
...
The **correlation** (a.k.a. correlation coefficient) between two random variables $X$ and $Y$ is 
$$
...
W_sd = sd(meeting_tactile_sim[, "W"]) * sqrt(9 / 10)
T_sd = sd(meeting_tactile_sim[, "T"]) * sqrt(9 / 10)
TW_cov = cov(meeting_tactile_sim[, "W"], meeting_tactile_sim[, "T"]) * 9 / 10
TW_corr = TW_cov / W_sd / T_sd

```

Returning to the $(T, W)$ example, we carry out the same process of computing the covariance, but first standardize each of the simulated values of $T$ by subtracting their mean and dividing by their standardization, and similarly for $W$.
Then we compute the product for each pair of standardized values and average the products to get the correlation  `{r} round(TW_corr, 3)`.



```{r}
#| label: tbl-sim-corr-calc
#| echo: false
#| tbl-cap: "Calculation of correlation based on 10 simulated values"

corr_calc_df = meeting_tactile_sim |>
  rename(repetition = 1) |>
  mutate(repetition = factor(repetition)) |>
  select(-c(R, Y)) |> 
  mutate(
    T_standardized = (T - mean(T)) / T_sd,
    W_standardized = (W - mean(W)) / W_sd,
    product_of_standardized_values = T_standardized * W_standardized
)
...
sd_row = data.frame(matrix(ncol = length(average_row), nrow = 0))
names(sd_row) = names(average_row)
sd_row[1, ] = c("SD", T_sd, W_sd, 1, 1, NA)


rbind(corr_calc_df, total_row, average_row, sd_row) |>
  kbl(digits = 3) |>
  kable_styling()
```
...
We'll use the `standardize` method to standardize the simulated values.
Note that values are standardized using the respective means and standard deviations.
...
We can also just call `corr` on the simulated $(T, W)$ pairs.
...
When standardizing, subtracting the means doesn't change the scale of the possible pairs of values or how far away they are from their means; it merely shifts the center of the joint distribution.
Dividing by the standard deviations does change the scale.
It can be shown that correlation is the covariance divided by the product of the standard deviations.


::: {#lem-correlation-shortcut}
$$
...
```{python}
t_and_w.cov() / (t_and_w.sd()[0] * (t_and_w.sd()[1]))
```

@def-correlation reflects the concept of correlation, but @lem-correlation-shortcut is usually how we compute correlation.
The hard work is in computing the covariance and the standard deviations; once we have those it's just a matter of dividing to get the correlation.


:::: {.callout-note appearance="simple"}
::: {#exm-corr-compute-dice}
In the dice rolling problem, let $X$ be the sum and $Y$ be the larger of the two rolls.

1.  Compute $\Corr(X, Y)$.
1.  Write Symbulate code to approximate $\Corr(X, Y)$ and compare to the values from part 2.

:::
::::

:::: {.callout-tip collapse=true}
::: {#sol-corr-compute-dice}

1.  In @exm-cov-compute-dice we computed $\Cov(X, Y) = 1.25$, and in @exm-sd-compute-dice we computed $\SD(Y) = 0.927$.
We just need to compute $\SD(X)$.
$\E(X)=5$, $\E(X^2) = 27.5$, $\Var(X) = 2.5$, and $\SD(X) = 1.581$.
Therefore
$$
\Corr(X, Y) = \frac{\Cov(X, Y)}{\SD(X)\SD(Y)} = \frac{1.25}{(1.581)(0.927)} = 0.853
$$
1.  See the code below (we simulated `x_and_y` pairs previously).
:::
::::
...
### Bivariate Normal distributions (briefly) {#sec-sim-bvn}
...
Donny Don't has just completed a problem where it was assumed that SAT Math scores follow a Normal(500, 100) distribution. 
Now a follow up problem asks Donny how he could simulate a single (Math, Reading) pair of scores. 
Donny says: "That's easy; just spin the Normal(500, 100) twice, once for Math and once for Reading."
Do you agree?
...
-   It's possible that the distribution of SAT Math scores follow a different pattern than SAT Reading scores. 
So we might need one spinner to simulate a Math score, and a second spinner to simulate the Reading score. 
(In reality, SAT Math and Reading scores do follow pretty similar distributions. 
But it's possible that they could follow different distributions.)
-   Furthermore, there is probably some relationship between scores. 
It is plausible that students who do well on one test tend to do well on the other. 
For example, students who score over 700 on Math are probably more likely to score above than below average on Reading. 
If we simulate a pair of scores by spinning one spinner for Math and a separate spinner for Reading, then there will be no relationship between the scores because the spins are physically independent.

:::
::::

In the previous problem, what we really need is a spinner that generates a pair of scores simultaneously to reflect their association.
...
-   The lines of latitude would not be equally spaced, since SAT Math scores are not equally likely. 
(We have seen similar issues for one-dimensional Normal spinners.) 
Similarly for lines of longitude.
-   The scale of the lines of latitude would not necessarily match the scale of the lines of longitude, since Math and Reading scores could follow difference distributions. 
For example, the equator (average Math) might be 500 while the prime meridian (average Reading) might be 520.
...
Recall that in @sec-language-simulation-computer-meeting-bvn we assumed that Regina and Cady tend to arrive around the same time.
One way to model pairs of values that have correlation is with a Bivariate Normal distribution, like in the following.
...
Simulated values of a random variable with a Normal distribution follow a "bell-shaped" curve.
Similarly, simulated pairs of values of two random variables with a Bivariate Normal distribution follow a "mound-shaped" or "fin-shaped" surface.



@fig-bivariate-normal-densities display a few Bivariate Normal distributions with different correlations.
Notice how the correlation changes the shape of the distribution.


```{r}
#| echo: false
#| label: fig-bivariate-normal-densities
#| fig-cap: "Bivariate Normal distributions"
#| warning: false
...
The condition for independence of random variables is fairly strict. 
Random variables are independent only if the joint distribution is determined by the product of the marginal distributions for all values of the random variables. 
Otherwise, the random variables are not independent. 
However, you can imagine that there are different degrees of dependence. 

We wish to measure degree of dependence in a single number. 
Unfortunately, there are many different ways two random variables can be related, and there is no single measure that works well in all cases.

The most commonly used measure of the degree of an association between two random variables is covariance, or its standardized counterpart, correlation.  
While covariance does have many uses and nice properties, it does NOT measure whether two random variables are independent.
...
1.  $X$ and $Y$ are independent in part 1 because the joint distribution is the product of the marginal distributions. 
However, $X$ and $Y$ in part 2 are not independent even though the covariance is 0.
...
Correlation is most natural as a measure of association for Bivariate Normal distributions.
If $X$ and $Y$ have a Bivariate Normal distribution and are uncorrelated, then $X$ and $Y$ are independent.
...
1.  Explain how you could, in principle, conduct a simulation by hand and use the results to approximate $\Cov(X, Y)$.
1.  Write Symbulate code to conduct a simulation and approximate the covariance and correlation.
1.  Compute $\Cov(X, Y)$ and $\Corr(X, Y)$.
...
1.  Compute $\text{E}(X)$.
1.  Compute $\Var(X)$.
1.  The joint distribution of $X$ and $Y$ is represented in the table below. 
Explain why $\IP(X=1, Y = 4) = 0$ and $\IP(X = 2, Y = 1) = 0$.
1.  Compute $\IP(X = 2, Y = 3)$. 
(Yes, the table tells you it's 0.1766, but show how this number can be computed based on the assumptions of the problem.)
1.  Are $X$ and $Y$ independent? 
...
(It can be shown that $\E(Y) = 5(18/38) = 2.37$)
1.  Compute $\Cov(X, Y)$.
Interpret the sign of the covariance in context.
1.  Compute $\Corr(X, Y)$.
(It can be shown that $\Var(Y) = 5(18/38)(20/38) = 1.25$)
...
## Simulating from distributions {#sec-simulate-from-distribution}
...
1.  Construct a single spinner for simulating $(X, Y)$ pairs with the proper joint distribution.
...
1.  See the spinner in @fig-dice-spinners-marginal-3 which would properly simulate $(X, Y)$ pairs according to the joint distribution.
...
#| echo: false
#| fig-cap: "Spinners representing the distributions of $X$ and $Y$ in @exm-dice-probspace"
#| fig-subcap: 
#|   - "The marginal distribution of $X$; see @tbl-dice-sum-dist-table"
#|   - "The marginal distribution of $Y$; see @tbl-dice-max-dist-table"
#|   - "The joint distribution of $X$ and $Y$; see @tbl-dice-joint-dist-twoway"
#| layout-ncol: 3
...
knitr::include_graphics(c("_graphics/spinner-dice-sum-max.png"))
...
Yes, this method does require that the distribution of $X$ is known.
However, in many situations we assume a distribution for a random variable directly, without specifying the underlying probability space.
This is what we did in the meeting problem whenever we assumed a Uniform(0, 60) or Normal(30, 10) distribution for arrival times.
Even Donny himself tried to implement the simulate from the distribution method in @exm-donny-sim-assume-independent.
...
1.  Can the code from the previous parts be combined to simulate $(X, Y)$ pairs?
1.  Write the simulate code to define $(X, Y)$ by specifying the joint distribution.
...
1.  We simulate a value of $X$ from its marginal distribution by spinning the spinner in @fig-dice-spinners-marginal-1, which we can define via a `BoxModel`
1.  Similarly, we simulate a value of $Y$ from its marginal distribution by spinning the spinner in @fig-dice-spinners-marginal-2, which we can define via a `BoxModel`
1.  No. The previous parts only define the marginal distributions, and marginal distributions alone do not define the joint distribution.
1.  We simulate an $(X, Y)$ pair from its joint distribution by spinning the spinner in @fig-dice-spinners-marginal-3, which we can define via a `BoxModel` whose tickets represent pairs.

:::
::::

We can define a `BoxModel` corresponding to each of the spinners in @fig-dice-spinners-marginal, and then define a random variable with that distribution via `RV(BoxModel(...))`.
Essentially, we define the random variable by specifying its distribution (spinner), rather than specifying the underlying probability space.
...
Many commonly encountered distributions have special names, and in Symbulate we can define a random variable to have a specific named distribution through code of the form `RV(Distribution(parameters))`; for example `X = RV(Uniform(0, 60))` or `X = RV(Normal(30, 10))`.
Think of code like `X = RV(Normal(30, 10))` as "$X$ is a random variable with a Normal(30, 10) distributon".
...
Roll two fair four-sided dice and let $U_1$ be the result of the first roll and $U_2$ the result of the second roll.
Then $U_1$ and $U_2$ have the same distribution; each takes values 1, 2, 3, 4 with equal probability.
However, these are two different random variables; one measure the first roll and one measure the second.
If the first roll is 4 and the second roll is 3, then $U_1=4$ and $U_2=3$, which are not the same.

Another discrete example: Flip a fair coin 3 times and let $X$ be the number of heads and $Y$ be the number of tails.
Then $X$ and $Y$ have the same distribution, because the coin is fair, roughly heads and tails would have the same long run pattern of variability.
(In this case each of $X$ and $Y$ takes values 0, 1, 2, 3 with probability 1/8, 3/8, 3/8, 1/8.)
But $X$ and $Y$ are not the same random variable; they are measuring different things.
If the outcome is HHT then $X$ is 2 but $Y$ is 1.
In this case $\IP(X = Y)=0$; in an odd number of flips it's not possible to have the same number of heads and tails on any single outcome.

A continuous example follows.
:::
::::

In some cases of the meeting time problem we assumed the distribution of Regina's arrival time $R$ is Uniform(0, 60) and the distribution of Cady's arrival time $Y$ is Uniform(0, 60), in which case $R$ and $Y$ have the same distribution.
...
:::: {.callout-note appearance="simple"}
::: {#exm-binomial-same-distribution}
Explain why each of the following random variables has the same distribution.
...
-   $W$ is the number of females in three human births, selected randomly and independently, assuming males and females are equally likely[^distribution-2]
...
:::: {.callout-tip collapse=true}
::: {#sol-binomial-same-distribution}
...
-   Each trial can be classified as "success"[^distribution-3] (Tails, even, female) or not.
...
For coins and dice the trials are physically independent. 
For births independence follows from random selection from a large population.
...
Each situation can be simulated by putting 2 tickets in a box labeled success or not, drawing 3 tickets with replacement, counting the number of successes drawn, and repeating many times.
All of these scenarios are probabilitistically the same, so each random variable would have the same long run pattern of variability.
...
For example, the common distribution in @exm-binomial-same-distribution is called the "Binomial(3, 0.5)" distribution.
...
@exm-binomial-same-distribution illustrates that knowledge that a random variable has a specific distribution (e.g., Binomial(3, 0.5)) does not necessarily convey any information about the underlying outcomes or random variable (function) being measured.


The scenarios involving $W, X, Y, Z$ in @exm-binomial-same-distribution illustrate that two random variables do not have to be defined on the same sample space in order to determine if they have the same distribution.
...
However, you could collect SAT Math scores for one set of students to estimate the marginal distribution of Math scores, and collect Reading scores for a separate set of students to estimate the marginal distribution of Reading scores, and see if those two marginal distributions are the same.
...
Suppose that $X$, $Y$, and $Z$ all have the same distribution. 
Donny Dont says
...
Determine if each of Donny's statements is correct. 
If not, explain why not using a simple example (say a coin flipping situation).
...
First of all, Donny's statements wouldn't even make sense unless the random variables were all defined on the same probability space. 
For example, if $X$ is SAT Math score and $Y$ is SAT reading score it doesn't makes sense to consider $X+Y$ unless $(X, Y)$ pairs are measured for the same students. 
But even assuming the random variables are defined on the same probability space, we can find counterexamples to Donny's statements.
...
1.  The joint distribution of $(X, Y)$ is not the same as the joint distribution of $(X, Z)$. 
For example, $(X, Y)$ takes the pair $(3, 3)$ with probability 0, but $(X, Z)$ takes the pair $(3, 3)$ with nonzero probability (1/16).
...
Comparing @exm-dice-probspace and @exm-dice-joint-independent provides another example of two scenarios with the same marginal distributions but different joint distributions.
...
Spin the Uniform(0, 1) spinner (see @fig-uniform-spinner) twice and let $U_1$ be the result of the first spin and $U_2$ the result of the second. 
For each of the following pairs of random variables, determine whether or not they have the same distribution as each other. 
No calculations necessary; just think conceptually.
...
1.  Yes, each has a Uniform(0, 1) distribution. 
For $u\in[0, 1]$, $1-u\in[0, 1]$, so $U_1$ and $1-U_1$ have the same possible values, and a linear rescaling does not change the shape of the distribution. 
Changing from $U_1$ to $1-U_1$ essentially amounts to switching the [0, 1] labels on the spinner from clockwise to counterclockwise.
