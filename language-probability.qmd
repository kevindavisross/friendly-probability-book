...
<!-- 
-   End of dice probability models: add back to two dice, include table for 16 outcomes with column for probability both with fair die and with the first weighted die, mention we'll discuss independence later
-   Independence section: refer back to the tables created in previous note and include example for how to compute the probability of each outcome, and add a few event probabilities for sum and max. Actually, for fair dice this should already be in intro to distribution section, so in independence section basically redo that for weighted die.
-->
...
### Counting outcomes {#sec-language-counting-outcomes}
...
### Marginal distributions do not determine the joint distribution
...
It is always possible to obtain marginal distributions of the random variables in a collection from their joint distribution.
However, in general the marginal distributions alone are not enough to determine the joint distribution.




:::: {.callout-note appearance="simple"}
::: {#exm-dice-joint-independent}
Suppose $X$ and $Y$ are random variables whose joint distribution is represented by @tbl-dice-joint-independent.

1.  Find the marginal distribution of $X$.
1.  Find the marginal distribution of $Y$.
1.  Are the marginal distributions of $X$ and $Y$ in this example the same as those in @exm-dice-probspace?
1.  Is the joint distribution of $X$ and $Y$ the same as the one in @exm-dice-probspace?

:::
::::

:::: {.callout-tip collapse=true}
::: {#sol-dice-joint-independent}
1.  The possible values of $X$ are 2, 3, 4, 5, 6, 7, 8.
Add a total column to the joint distribution table to find the probabilities of each possible $x$ value, which are, respectively, 16/256, 32/356, 42/256, 64/256, 42/256, 32/356, 16/256, or 1/16, 2/16, 3/16, 4/16, 3/16, 2/16, 1/16. 
1.  The possible values of $Y$ are 1, 2, 3, 4.
Add a total row to the joint distribution table to find the probabilities of each possible $y$ value, which are, respectively, 16/256, 48/356, 80/256, 112/256, or 1/16, 3/16, 5/16, 7/16. 
1.  Yes, the marginal distributions of $X$ and $Y$ are the same as in @exm-dice-probspace, represented by @tbl-dice-sum-dist-table and @tbl-dice-max-dist-table.
1.  No, this is a different joint distribution than the one in @tbl-dice-joint-dist-twoway.
For example, in @tbl-dice-joint-dist-twoway some of the $(X, Y)$ pairs aren't even possible, but the joint distribution in @tbl-dice-joint-independent assigns positive probability to all $(X, Y)$ pairs.
:::
::::
...
: The joint distribution for @exm-dice-joint-independent {#tbl-dice-joint-independent}




@tbl-dice-joint-dist-twoway and @tbl-dice-joint-independent provide an illustration of two different joint distributons with the same marginal distributions.
When representing the joint distribution of two discrete random variables in a table, just because you know the row and column totals doesn't mean you know all the values of the interior cells.

A joint distribution represents all of the probabilistic behavior of a collection of random variables.
It is always possible to obtain marginal distributions of the random variables in a collection from their joint distribution.

However, in general you cannot determine the joint distribution based on the marginal distributions alone.
Marginal distributions only reflect how each random variable behaves in isolation.
The joint distribution goes further and fully represents relationships between the random variables.
Just because you know how each random variable behaves individually, you don't necessarily know how they behave in relationship with each other.

::: {.callout-warning appearance="default"}
In general, marginal distributions alone are not enough to determine a joint distribution. 
:::

The exception to this warning is when random variables are *independent*, which we'll discuss later.
But you shouldn't simply assume random variables are independent without sufficient justification.
...
ggplot(df, aes(x = n, y = pn)) +
  geom_point() +
  annotate("segment", x = n0, xend = n0, y = 0, yend = df$pn[n0],
           linetype = "dashed", linewidth = 1, color = "skyblue") +
  annotate("segment", x = 0, xend = n0, y = df$pn[n0], yend = df$pn[n0],
           linetype = "dashed", linewidth = 1, color = "skyblue") +
  labs(
...
In which of the three scenarios represented in @fig-venn-conditional-plot are events $A$ and $B$ independent?
...
:::: {.callout-note appearance="simple"}
::: {#exm-dice-joint-independent-conditional}
Suppose $X$ and $Y$ are random variables whose joint distribution is represented by @tbl-dice-joint-independent.
Recall that we found the marginal distributions in @exm-dice-joint-independent.

1.  How is $\IP(X = 2, Y=1)$ related to $\IP(X = 2)$ and $\IP(Y=1)$?
What does this imply?
1.  How is the joint distribution of $X$ and $Y$ related to the marginal distributions of $X$ and $Y$?
1.  Compute the conditional distribution of $Y$ given $X=2$.
How does it compare to the marginal distribution of $Y$?
1.  Compute the conditional distribution of $Y$ given $X=x$ for each possible $x$.
What do you notice?
1.  Compute the conditional distribution of $X$ given $Y=1$.
How does it compare to the marginal distribution of $X$?
1.  Compute the conditional distribution of $X$ given $Y=y$ for each possible $y$.
What do you notice?
...
:::: {.callout-tip collapse=true}
::: {#sol-dice-joint-independent-conditional}
1.  The joint probability is the product of the marginal probabilities, $\IP(X = 2, Y=1)= 1/256 = (1/16)(1/16) = \IP(X = 2) \IP(Y=1)$, so the events $\{X=2\}$ and $\{Y=1\}$ are independent. 
1.  For each possible value $x$ of $X$ and $y$ of $Y$, $\IP(X = x, Y=y)= \IP(X = x) \IP(Y=y)$.
In short, the joint distribution of $X$ and $Y$ is the product of their marginal distributions.
1.  Slice and renormalize: the possible values of $Y$ are 1, 2, 3, 4 with respective probabilities 1/6, 3/16, 5/16, 7/16.
The conditional distribution of $Y$ given $X=2$ is the same as the marginal distribution of $Y$.
1.  Along each possible $x$ slice, the possible values of $Y$ are 1, 2, 3, 4 with respective probabilities 1/6, 3/16, 5/16, 7/16.
For every possible value $x$ of $X$, the conditional distribution of $Y$ given $X=x$ is the same as the marginal distribution of $Y$.
1.  Slice and renormalize: the possible values of $X$ are 2, 3, 4, 5, 6, 7, 8 with respective probabilities 1/16, 2/16, 3/16, 4/16, 3/16, 2/16, 1/16.
The conditional distribution of $X$ given $Y=1$ is the same as the marginal distribution of $X$.
1.  Along each possible $y$ slice, the possible values of $X$ are 2, 3, 4, 5, 6, 7, 8 with respective probabilities 1/16, 2/16, 3/16, 4/16, 3/16, 2/16, 1/16.
For every possible value $y$ of $Y$, the conditional distribution of $X$ given $Y=y$ is the same as the marginal distribution of $X$.
1.  Yes, the random variables $X$ and $Y$ independent.
For every possible value $x$ of $X$ and $y$ of $Y$ the events $\{X=x\}$ and $\{Y=y\}$ are independent.
Knowing the value of one variable does not change the distribution of the other.
:::
::::
...
@fig-independent-rv-mosaic displays mosaic plots of the distributions of the two independent discrete random variables of @exm-dice-joint-independent-conditional.
Notice that the conditional distribution of $X$ is the same for each value of $Y$, and vice versa.
...
knitr::include_graphics(c("_graphics/mosaic-dice-sum-max-independent-given-x.png", "_graphics/mosaic-dice-sum-max-independent-given-y.png"))
