...
In this case, the event $\{3, 1\}$ would be the event that the single roll results in either either a 3 or a 1.
(The set $\{3, 1\}$ is the same as the set $\{1, 3\}$.)
...
@tbl-die-events lists the collection of all events ($\mathcal{F}$), and whether they occur if the single roll results in a 3 (that is, for the outcome $\omega=3$).
...
Now we will explore conditioning in more detail, introducing some of the notation and math.
...
(This value was provided to use directly in @exm-conditional-probability-def.)
...
1.  The third card adds a third "stage" but the multiplication rule extended naturally.
...
1.  For $n=30$, cind the probability that at least two people have the same birthday.
...
1.  Now consider $n=4$.
In order for all 4 people to have different birthdays, the first three people have to have different birthdays and the fourth also has to be different from theirs.
    ...
    -   Using the multiplication rule, the probability that the first three people have to have different birthdays and the fourth is also different from theirs is $(364/365)(363/365)(362/365) = 0.983$
...
That probability is the probability that *at least two people* in the group of 100 share a birthday.
...
If $C_1, C_2, C_3\ldots$ are disjoint events with $C_1\cup C_2 \cup C_3\cup \cdots =\Omega$, then^[Proof: start with Lemma @lem-ltp1 and use the multiplication rule to write $\IP(A \cap C_1)=\IP(A|C_1)\IP(C_1)$, etc.]
:::
\begin{align*}
...
1. Use the joint probabilities we computed in @exm-conditional-probability-def-slicing; see the "uses Snapchat" column of the two-way table in the solution to part 4.
...
1. Use the conditional probabilities we computed in @exm-conditional-probability-def-slicing; see the solution to part 7.
...
Renormalizing only changes the absolute scale of the plot; compare the values on the vertical axes, which correspond to joint probabilities on the left and conditional probabilities on the right.
...
1. Left: $\IP(A|B)=0$.
After conditioning on $B$, there are now 4 equally likely outcomes, of which none satisfy $A$.
...
Identify the prior probability, hypothesis, evidence, likelihood, and posterior probability, and use Bayes' rule to compute the posterior probability. 
...
In @tbl-bayes-rule-iterative-bayestable the evidece is that the American has a postgraduate degree; the likelihood column contains the probability of the same event, $E$ = "the American has a postgraduate degree", under each of the distinct hypotheses:
...
Be careful to distinguish between mosaic plots and tile plots.
...
We will explore conditioning in more detail throughout the book, including conditional distributions when continuous continuous random variables are involved.
...
1.  The drawings are independent.
...
The probability of winning any two drawings is about 1 in 85 quadrillion.
