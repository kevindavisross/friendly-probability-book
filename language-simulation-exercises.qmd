...
1.  See previous solutions for how to simulate $(X, Y)$ pairs.
    -   Simulate many $(X, Y)$ pairs
    -   Compute the average of the simulated $X$ values (i.e., sum the simulated values of $X$ and divide by the number of simulated values) to approximate $\E(X)$
    -   Compute the average of the simulated $Y$ values (i.e., sum the simulated values of $Y$ and divide by the number of simulated values) to approximate $\E(Y)$
    -   Find the product $XY$ for each pair, and compute the average value of the products (i.e., sum the simulated values of $XY$ and divide by the number of simulated pairs) to approximate $\E(XY)$
    -   Subract the approximations $\E(XY) - \E(X)\E(Y)$ to approximate $\Cov(X, Y)$
1.  See code below
1.  In @exr-distribution-intro-collector3 we found the joint distribution of $X$ and $Y$ in a previous problem, and also computed $\E(X) = 57/27 = 2.111$ and $\E(Y)=1$.
In computing $\E(XY)$ many of the terms are 0, either because one of $Y$ is 0 or because the probability of the pair is 0
$$
\E(XY) = (1)(3)(1/27) + (2)(1)(6/27) + (2)(2)(6/27) + (3)(1)(6/27) + 0 = 57/27 = 2.111
$$
So $\Cov(X, Y) = \E(XY) - \E(X)\E(Y) = 57/27 - (57/27)(1) = 0$.
Therefore $\Corr(X, Y) = 0$ and $X$ and $Y$ are uncorrelated.
1.  Even though the covariance is 0, $X$ and $Y$ are not independent.
For example $\IP(X = 1) = 3/27$ but $\IP(X = 1|Y=1) = 0$.
Knowing the value of $Y$ changes the distribution of $X$, and vice versa.
...
One outcome corresponds to each of $X=0$ (LLL) and $X=3$ (WWW), and 3 outcomes correpond to each of $X=1$ (WLL, LWL, LLW) and $X=2$ (WWL, WLW, LWW), but the outcomes are not equally likely.

    | $x$ | Outcomes      |                  $\IP(X = X)$ |
    |-----|---------------|------------------------------:|
    | 0   | LLL           | $(1)(18/38)^0(20/38)^3=0.146$ |
    | 1   | WLL, LWL, LLW | $(3)(18/38)^1(20/38)^2=0.394$ |
    | 2   | WWL, WLW, LWW | $(3)(18/38)^2(20/38)^1=0.354$ |
    | 3   | WWW           | $(1)(18/38)^3(20/38)^0=0.106$ |

1.  $\text{E}(X)=0(0.146)+1(0.394)+2(0.354)+3(0.106)= 1.42$.
1.  $\text{E}(X^2)=0^2(0.146)+1^2(0.394)+2^2(0.354)+3^2(0.106)= 2.76$, so $\Var(X)=0.75$.
...
1.  To have $X = 2$ and $Y=3$ we need to have exactly two wins in the first 3 bets (where both play) and exactly 1 win in the last 2 bets (Yolanda only).
    $$
    \text{P}(X = 2, Y = 3) = \left((3)(18/38)^2(20/38)^1\right)\left((2)(18/38)^1(20/38)^1\right) = 0.177
    $$
1.  $X$ and $Y$ are not independent.
For example, $\IP(X = 1, Y = 0)=0$ which is not $\IP(X=1)\IP(Y=0)$. 
1.  $\IP(X + Y = 4) = 0.0883+0.0981 = 0.1864$.
1.  Sum the appropriate values from the joint distribution table.
...
1.  Use the table from the previous part to compute $\text{E}(X+Y) = 3.79$.
1.  $\E(XY) = 4.11$ so $\Cov(X, Y) =  4.11 - (1.42)(2.37) = 0.75$.
There is a positive association; if $X$ is large that means Xavier won a lot of games so Yolanda tends to win more games too.
1.  $\Corr(X, Y) = 0.75/\sqrt{(0.75)(1.25)} = 0.77$.
